...
With this in place we only need to make sure to include the RTL files needed for the build and do the build process. In this tutorial, we have several parts in place already but we'll guide you through which components need to be modified. 

## Adding RTL Files

All RTL filelists are generated through [bender](https://github.com/pulp-platform/bender). Bender is a file management tool that functions like git submodules but with more features like file lists generation in different forms. 

We first need to make sure we add all the files we need for our SNAX ALU. At the root of the repository you should find a `Bender.yml` file. You will see several files related to the cluster. Scroll down to almost at the bottom of the file and you should see the `SNAX Accelerators` section. You should also find the SNAX ALU in there:
...
The files are listed according to a hierarchical structure from the base modules up to the system modules. This means that Level 0 contains modules that are not dependent on any other modules. Level 1 are dependent on Level 0 modules. Likewise, Level 2 are dependent on both Level 1 and Level 0 modules.
...
When you make your own accelerator you need to declare it all the RTL files in this format too. Make sure it's under the SNAX Accelerator section just to make sure you don't mess up the structure.

### What About the Naming of the Generated Files?

The name of the generated files is dependent on the configuration file you set. If you recall in the `snax_acc_cfg`, there is a `snax_acc_name` parameter: 
...
```
As you can see, we need to specify `CFG_OVERRIDE=cfg/snax-alu.hjson` later when we build the file because the conditional statement checks the name of the configuration file. The `SNAX_GEN += ${GENERATED_DIR}/snax-alu` sets the target directory for where the build will be placed. The parameters with `*_BENDER += -t snax-alu` tells the build to use bender with the `-target snax-alu` defined. This is in accordance to the `-target` parameter mentioned in the Bender file.
...
3 - Wait 5 to 10 mins until the build finishes.
...
In principle, both scripts use a specified configuration file like the `snax-alu.hjson` and loads those configurations in a templated file `*.tpl`. Once loaded, it generates the target files like System Verilog designs or Chisel parameters.

For example, the **accelerator wrapper** script uses any of the configuration files under the `./target/snitch_cluster/cfg/.` directory. Then loads them in wrapper templates found in `./hw/templates/.` directory.
...
The **cluster wrapper** made by the Snitch platform also does the same mechanism but tailored towards the system integration. The main template is in `./hw/snitch_cluster/src/snitch_cluster_wrapper.sv.tpl`. To run the `clustergen` do:
...
Let's go through the `snax-alu.hjson` file again and look at important configurations. We'll only discuss the notable ones that would be most useful for an accelarator designer. Remember, you can locate this at `./target/snitch_cluster/cfg/snax-alu.hjson` starting from the root of the repository.
...
Then we have DMA parameters for the built-in Snitch DMA. The `dma_data_width` affects the DMA bandwidth and also the wide interconnect of the cluster. This is a special parameter that can be useful if you want to hook your accelerator towards the wide interconect. The FIFOs are for handling buffering inside the DMA.
...
For a multi-accelerator system attached in one cluster, we can have the core configuration like the one below. This time it has 3 other accelerators within one cluster.
