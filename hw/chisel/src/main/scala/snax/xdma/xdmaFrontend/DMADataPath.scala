...
class DMADataPath(readerparam: ReaderWriterDataPathParam, writerparam: ReaderWriterDataPathParam)
  ...
  if (readerparam.extParam.length == 0) {
    ...
    io.reader_busy_o := reader.io.busy | (~reader.io.bufferEmpty) | (readerparam.extParam.map { _.ext_busy_o }
      .reduce(_ | _))
  ...
  if (writerparam.extParam.length == 0) {
    ...
    io.writer_busy_o := writer.io.busy | (~writer.io.bufferEmpty) | (writerparam.extParam.map { _.ext_busy_o }
      .reduce(_ | _))
